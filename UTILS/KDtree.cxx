...
KDNode::KDNode(const point_t& pt, size_t const& idx,
    KDNodePtr const& left_ptr, KDNodePtr const& right_ptr)
    : index(idx), x(pt), left(left_ptr), right(right_ptr) {}
...
double KDNode::coord(size_t const& idx) { return x.coords.at(idx); }
KDNode::operator bool() { return (!x.coords.empty()); }
...
inline double dist2(point_t const& a, point_t const& b) {
    ...
    for (size_t i = 0; i < a.size(); i++) {
        double di = a.coords.at(i) - b.coords.at(i);
        distc += di * di;
    }
    return distc;
}
...
inline bool comparer::compare_idx(pointIndex const& a, pointIndex const& b) {
    return (a.first.coords.at(idx) < b.first.coords.at(idx));
}
...
void KDTree::knearest_(
    ...
    double const dx = branch_pt.coords.at(level) - pt.coords.at(level);
...
void KDTree::neighborhood_(KDNodePtr const& branch, point_t const& pt,
    ...
    double const dx = static_cast<point_t>(*branch).coords.at(level) - pt.coords.at(level);
    double const dx2 = dx * dx;
